...
As discussed in detail in Section \ref{sec:had_calo}, quarks and gluons hadronize in the hadronic calorimeter to produce showers of energy deposits in the calorimeter known as jets. This search uses the particle flow algorithm \cite{PERF-2015-09} to reconstruct objects associated with the resulting energy deposits. The particle flow algorithm matches signals from the inner tracker and the topologically connected clusters of energy deposits in the calorimeter with the aim of forming objects representing individual charged particles The anti-\(k_t\) algorithm is employed to reconstruct the resulting clusters of energy deposits in the hadronic calorimeter using topologically connected clusters of energy deposits (a.k.a. topo-clusters) in the calorimeter. Various radii \(R\) may be considered for jet reconstruction depending on the kinematics and origins of the quark or gluon which initiated the shower (see discussion in Section \ref{sec:had_calo} for more details, where \(R\) determines the angular radius within which the anti-\(k_t\) algorithm includes calorimeter deposits for jet reconstruction. 

As discussed in Chapter \ref{chapter:dh_model}, the DH model signature targeted in this search involves a pair of energetic \(W\) bosons in the final state, where one decays leptonically and the other hadronically to a pair of quarks. If the boost of the hadronically decaying \(W\) is sufficiently low, the two quarks may have a large enough angular separation as to be most effectively reconstructed as two separate jets with small radius, and it will not be possible to collectively reconstruct them as a single two-pronged large-radius jet. In this so-called ``resolved" regime, the individually resolved ``\SmallR" jets are reconstructed for this search with a radius of \(R=0.2\). 

\SmallR jets are reconstructed by clustering particle flow objects \cite{PERF-2015-09} using the \akt algorithm \cite{Cacciari:2008gp} within an angular radius of \(R=0.4\). After they are fully calibrated \cite{ATLAS-CONF-2015-037}, only jets with \(\pt > 20~\GeV\) and \(|\eta| < 2.5\) are considered for the search. Jet cleaning \cite{ATLAS-CONF-2015-029} with the \verb|TightBad| working point is applied to suppress noise in the calorimeter and background jets which are not produced from the primary collision. The jet vertex tagger \cite{ATLAS-CONF-2014-018} is applied with the \verb|Tight| working point to suppress ``pile-up" jets from other \(pp\) interactions in the same and neighbouring bunch crossings. These jets are used in the analysis to identify the hadronic decay products of a \(W\) boson in the \resolved case. 

\subsubsection*{\btag}
\btagged jets are identified with the \verb|DL1r| algorithm \cite{ATLAS-CONF-2018-006}, which uses a deep learning method for the identification. A fixed working point with 77\% efficiency is used. \btagged jets are vetoed in this analysis.
