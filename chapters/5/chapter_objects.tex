...
\noindent where \(W_\text{had}\) denotes the hadronically decaying \(W\) and \(W_\text{lep}\) denotes the leptonically decaying \(W\). The \(W_\text{had}\) four-momentum is reconstructed in the resolved regime using the pair of \akt4 jets whose invariant mass is closest to the on-shell \(W\) mass of \(80.4~\GeV\) (see Section \ref{sec:resolved_w_cand}), or as the four-momentum of the highest-\pt \(R=1.0\) TAR jet in the merged regime (see Section \ref{sec:TAR_jets}). The four-momentum of the \(W_\text{lep}\) is the sum of four momenta of the lepton and neutrino that it decays to:
