...
\newline \newline \emph{Baseline} electrons satisfy a relatively loose set of identification criteria, referred to in ATLAS. In addition, a loose isolation is required. This electron definition has a high efficiency, and is used to veto the presence of additional electrons in the final state.
\newline \emph{Signal} electrons are designed to be high-purity electrons. They are required to satisfy the \emph{medium} identification and loose isolation criteria.
\newline \newline Both types of electrons use a threshold of \(\pT > 7{\GeV} \) and a pseudorapidity range of \(|\eta| < 2.47\).
