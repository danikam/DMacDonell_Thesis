...
An approximate solution is obtained by assuming that the DH is only as massive as is kinematically required in order to produce the observed final state, subject to the constraint that the invariant mass of the reconstructed \(\mathbf{p}_{W_\text{lep}}\) be equal to the on-shell \(W\) mass of \(80.4~\GeV\). The assumption of minimal DH mass \ms is motivated by the fact that the lower the mass of a mediator, the larger is the range of mediator momenta for which the decay of the mediator can produce a given final state for which the momentum of one of the final-state particles (the neutrino) is unknown but constrained (i.e. lying somewhere between 0 and the measured \met). Therefore, for a given final state, the lowest mediator mass that can kinematically produce the final state has the largest space of momenta available by which it could do so compared with larger candidate mediator masses, and hence the highest likelihood of being the actual mediator mass.
...
\noindent under the assumptions that the masses associated with the lepton and \met are negligibly small compared with their momenta. The assumption of negligible lepton mass is in general justified given the energy of LHC collisions. The assumption of negligible mass associated with \met is justified if the true \met arises only from the neutrino in the leptonic \(W_\text{lep}\) decay, as it would in the leading SM backgrounds. In the signal model, however, there is additional mass associated with the \met arising from DM production. The result, shown in figure \ref{fig:mT_lep_met} after applying the pre-selection cuts, is that the bulk of the SM background has \mtlepmet below the W mass peak, but the signal distribution tends to be peaked closer to \(\sim 250~\GeV\). 
...
The analysis selection is divided into two kinematic categories. The selections within each of the signal and control regions are divided into two 

, we distinguish two categories describing the boost of the dijet system: \merged and \resolved. The \merged region is designed to target the regime in which the dijet system is sufficiently boosted as to be reconstructed as a single \largeR TAR jet. The \resolved region targets the low-boost regime in which the two jets in the final state can be individually resolved as \smallR \akt \(R=0.4\) jets.

\Fig{\ref{fig:categories}} illustrates the two kinematic categories graphically.
