...
After applying the baseline selections and \bjet veto, the placement of upper and lower bounds is optimized for the selection variables listed in Table \ref{tab:opt_vars}. The optimization was performed first in the merged category of the SR, with the additional requirement of at least one \(R=1.0\) TAR jet. Once the selections defining the merged SR were finalized, optimization was subsequently in the resolved category, with the \(\NTAR > 0\) requirement replaced by \(\Njets > 1\) in addition to a veto on any events which pass the finalized merged SR selections. 

Since the optimal placement of selections was found to vary to some extent for MC simulated data sets with different \ms and \mZp, the cut placements were initially optimized using the Asimov discovery significance averaged over MC simulated data sets at the following four mass points which cover most of the \ms range considered in the search: (\ms, \mZp) = \{(210, 2100), (285, 1700), (310, 500), (335, 1000)\} GeV.  
...
\toprule
\textbf{Variable} & \textbf{Description} & \textbf{Used in Merged?} & \textbf{Used in Resolved?} \\
\midrule
\midrule
\met & See description in Section \ref{sec:met_object_description}. & \checkmark & \checkmark \\
\midrule
\metsig & See description in Section \ref{sec:met_object_description}. & \checkmark & \checkmark \\
\midrule
\mTAR & Reconstructed mass of the highest-\pt \(R=1.0\) TAR jet. & \checkmark & \(\times\) \\
\midrule
