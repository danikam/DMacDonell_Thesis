...
\begin{itemize}
\item this is a signature-driven search (the DH model is just one possible first-order model for a mechanism that could produce a signature in this final state).
\item and the production rate is expected to similarly scale with \(\gchi^2\), which is not constrained by dijet searches
\end{itemize}
