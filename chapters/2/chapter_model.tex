...
%		 \end{tikzpicture}
	...
	\end{subfigure}
	\caption[Leading Feynman diagrams for hypothetical DM production at the LHC by means of the DH model.]{Feynman diagrams with leading contributions to the cross section of hypothetical DM production at the LHC by means of the DH model.}
...
\begin{figure}[hp]
	...
	\caption[Global scans of the DH model, with \Zprime and DH as portal mediators, performed by Duerr et al.]{Global scans of the DH model, with \Zprime and DH as portal mediators, performed by Duerr et al. in Ref. \cite{Duerr_2016}. The red shaded region is excluded for all possible
...
\noindent where \(H=\frac{1}{\sqrt{2}}(0, h+v)\) represents the SM Higgs field with vacuum expectation value \(v\), and \(S=\frac{1}{\sqrt{2}}(s+w)\) represents the DH field. The third term with coupling \(\lambda_{hs}\) mixes the DH and SM Higgs fields, such that the physical mass eigenstates \(h'\) and \(s'\) are a superposition of the scalars \(h\) and \(s\):
...
\begin{subfigure}[t]{0.53\textwidth}
	\centering
	\includegraphics[width=0.99\textwidth]{Figures/2/Dijet_limits_ATLAS.pdf}
...
\end{subfigure}
	\caption[Upper bounds on the \(g_q\) coupling between the \Zprime and quarks from recent dijet resonance searches by ATLAS and CMS.]{Upper bounds on the \(g_q\) coupling between the \Zprime and quarks, as reported by recent dijet resonance searches performed by ATLAS (left) and CMS (right).}
...
\begin{table}[hp]
\centering
\caption[Summary of Dark Higgs decay channels to Standard Model particles.]{Summary of DH decay channels to SM particles that could be targeted to probe various \ms ranges in DH model.}
...
\toprule
\multirow{2}{*}  \textbf{\textbf{Decay channel}} & \textbf{Branching fractions \cite{pdg_2020}} & \textbf{Search effort(s)} \\
...
\end{subfigure}
	\caption[Range of \((\ms, \mZp)\) in the DH model excluded by searches performed by ATLAS and CMS.]{Range of \((\ms, \mZp)\) in the DH model excluded by searches performed by ATLAS in the \(s\rightarrow VV(q\bar{q}q\bar{q})\) channel (left), and by CMS in the \(s\rightarrow WW(\ell\nu\ell\nu)\) channel (right), for the following choices of remaining parameters in the model: \(g_q=0.25\), \(\gchi=1\), \(\sin\theta=0.01\), \(\mchi=200~\GeV\).}
