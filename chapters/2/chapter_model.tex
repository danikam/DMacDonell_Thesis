...
A new gauge group called the U(1)' with an associated vector gauge boson referred to as the \Zprime is introduced as an extension of the SM gauge group presented in Chapter \ref{chapter:introduction}. Both the DM and and teh \Zprime are assumed to obtain their mass from a new Higgs field, which gives rise to a new physical Higgs boson, the DH boson. The DM acquires an axial coupling to the \Zprime, such that all three dark sector particles interact with one another. The model considers a scenario in which the SM quarks are charged under the U(1)' gauge group, such that they can interact with the associated \Zprime vector boson, thus opening up a vector-mediated portal interactions between the SM and the dark sector. The model further postulates a Higgs-mediated portal interaction wherein the DH can decay to SM particles via a small mixing with the SM Higgs boson, with mixing angle \(\theta\). 
...
\item For god's sake give some detail on this DH-SM Higgs mixing...
